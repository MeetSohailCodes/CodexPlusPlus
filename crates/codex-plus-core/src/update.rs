...
pub const DEFAULT_REPOSITORY: &str = "MeetSohailCodes/CodexPlusPlus";
pub const DEFAULT_LATEST_JSON_URL: &str =
    "https://github.com/MeetSohailCodes/CodexPlusPlus/releases/latest/download/latest.json";
...
pub fn launch_installer(path: &Path) -> anyhow::Result<()> {
    #[cfg(windows)]
    {
        ...
        std::process::Command::new(path)
            ...
            .spawn()
            .map(|_| ())
            .map_err(|error| anyhow::anyhow!("Failed to launch installer: {error}"))
    }
    ...
    {
        let _ = path;
        anyhow::bail!("Current platform does not support launching installers")
    }
}
